...
def line_detection(image):
    ...
    horizontalStructure = cv2.getStructuringElement(cv2.MORPH_RECT, (30, 1))
    ...
    horizontal = cv2.erode(horizontal, (1,1), iterations=8)
    ...
    hor_lines = cv2.HoughLinesP(horizontal,rho=1,theta=np.pi/180,threshold=100,minLineLength=30,maxLineGap=20)
    ...
    ver_lines = cv2.HoughLinesP(vertical, 1, np.pi/180, 40, np.array([]), 60, 20)
    ...
    for x1, y1, x2, y2 in ver:
        cv2.line(post, (x1,y1), (x2,y2), (0, 0, 0), 1)

    for x1, y1, x2, y2 in hor:
        cv2.line(post, (x1,y1-4), (x2,y2-4), (0, 0, 0), 1)
...
image = cv2.imread('Broken\\results_3.jpg')
print(image.shape)
